...
class HangoutCoreBot(commands.Bot): # Sub class bot so we can have more customizability
    ...
    BotSynced = False
    ViewsAdded = False
    intents = BotUtils.bot.GetIntents()
    activity = BotUtils.bot.GetActivity()
    def __init__(
        self,
        *args,
        command_prefix, 
    ...
    async def setup_hook(self) -> None:
        ...
        for file in self.initial_extensions["valid_files"]:
            ...
            try:
                await self.load_extension(f'{config.COG_DIRECTORY_PATH}.{file[:-3]}')
            except Exception as e:
                BotUtils.errorprocessing.CogLoadError(file, e, self.debug_mode)
            else:
        ...
        terminal.log.INFO(f"Successfully loaded {len(self.initial_extensions['valid_files'])} extension(s).")
        terminal.log.INFO(f"Logged in as {self.user} (ID: {self.user.id}).")
        if len(self.initial_extensions["invalid_files"]) > 0:
            terminal.log.WARNING(f"Found {len(self.initial_extensions['invalid_files'])} invalid extension(s) in the 'cogs' directory. If you believe this is an error please verify each .py file and make sure it is set up as a cog properly, Otherwise you can ignore this message.")
        ...
        if opuslib is not None:
            try:
                ...
                if not discord.opus.is_loaded():
                    terminal.log.CRITICAL("Opus Failed To Load. It's corn! A big lump with knobs. It's got the juice!")
                else:
    ...
    async def on_ready(self):
        ...
        for guild in self.guilds:
            await BotUtils.database.RegisterGuild(guild)
...
async def main():
    ...
    async with ClientSession() as our_client:
        ...
        cogs = BotUtils.local.GetCogs()
        async with HangoutCoreBot(command_prefix=BotUtils.bot.GetPrefix, web_client=our_client,testing_guild_id=cfg["bot"]["developer_guild_id"], initial_extensions=cogs,terminal_args=sys.argv[1:]) as bot:
            if isinstance(cfg["bot"]["token"], str) and cfg["bot"]["token"] != "":
                ...
                terminal.log.CRITICAL(f"No token was provided in '{BotUtils.config.CONFIG_PATH}'")
