...
class Confirm(discord.ui.View):
    def __init__(self, confirmation: str =  "Confirming.", cancellation: str = "Cancelling."):
...
class PaginationView(discord.ui.View):
        def __init__(self, data, interaction: discord.Interaction, timeout: Optional[float] = 180):
        ...
        def createEmbed(self):
            ...
            _embed.set_footer(text=f"Page: {self.currentPage+1} of {len(self.pages)} - {self.embedFooter}")
            ...
            if _pF is not None:
                ...
                _embed.set_footer(text = f"Page: {self.currentPage+1} of {len(self.pages)} - {_pF}")
            else:
                _embed.set_footer(text = f"Page: {self.currentPage+1} of {len(self.pages)} - {self.embedFooter}")
