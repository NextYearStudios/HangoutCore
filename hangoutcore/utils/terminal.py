...
class Terminal(object):
    ...
    class ExtensionAlreadyLoadedError(Exception):
        ...
        def __init__(self, extension, message=None):
            if message is None:
                message = (
                    f"Failed to load extension '{extension}', It is already loaded."
                )
    ...
    class Log(object):
        ...
        def CRITICAL(self, log: str):
            ...
            if self.__checklevel__(logging.CRITICAL) and not self.silent:
                self.console.print(
                    f"[{self.colors['critical']}]{_prefix} {log} {_suffix}",
                    justify="left",
                )
    ...
    def print_center(self, message: str, fill_char: str = " "):
        self.print(
            message.center(shutil.get_terminal_size().columns, fill_char),
            alt=f"{message}",
        )
    ...
    def input(self, prompt: TextType, **kwargs):
        if not self.silent:
            self.logger.debug(
                f"Requested user input with the following prompt: {prompt}"
            )
            try:
            ...
            self.logger.critical(
                "Input required, Could not request input in silent mode."
            )
            sys.exit(1)
    ...
    def confirm(self, prompt: object, **kwargs):
        if not self.silent:
            self.logger.debug(
                f"Requested user confirmation with the following prompt: {prompt}"
            )
            ...
            self.logger.critical(
                "Confirmation required, Could not request input in silent mode."
            )
            sys.exit(1)
