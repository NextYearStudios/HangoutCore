"""
    ...
    Module Last Updated: November 22, 2022
...
class local():
    ...
    async def load_extensions(discordBot:commands.Bot, debug_mode:bool=False):
        """
        ...
        for cog in cogs["valid_files"]:
            ...
            try:
                terminal.log.INFO(f'{os.getcwd()}/{config.COG_DIRECTORY_PATH}/{cog}')
                #terminal.log.INFO(f'{importlib.util.spec_from_file_location(f"{config.COG_DIRECTORY_PATH}.{cog[:-3]}", f"../{config.COG_DIRECTORY_PATH}/{cog}")}')
                # await discordBot.load_extension(importlib.util.spec_from_file_location(f"{config.COG_DIRECTORY_PATH}.{cog[:-3]}", f"../{config.COG_DIRECTORY_PATH}/{cog}"))
                await discordBot._load_from_module_spec(importlib.util.spec_from_file_location(f"{config.COG_DIRECTORY_PATH}.{cog[:-3]}", f"{config.COG_DIRECTORY_PATH}/{cog}"), f"{config.COG_DIRECTORY_PATH}.{cog[:-3]}")
            except Exception as e:
                terminal.errorprocessing.CogLoadError(cog, e, True)
            else:
