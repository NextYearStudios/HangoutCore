...
def __init__():
    ...
    def start_main_process():
        process = subprocess.Popen(
            [
                f"{sys.executable}",
                f"{os.path.dirname(os.path.abspath(__file__))}\\__main__.py",
            ],
            cwd=os.getcwd(),
            # encoding="UTF-8",
            env=os.environ.copy(),
        ...
        for line in process.stdout:
            ...
            terminal.console.print(processed_line)
        process.wait()
    ...
    def process_args(argv: list[str]):
        ...
        def get_arg(short_arg: str, long_arg: str) -> str | bool | int:
            if argv is None:
                ...
                _arg_position = None
                _return = {"exists": False, "value": None}
        ...
        if _arg_help["exists"]:
            ...
            terminal.print(
                f"[b][[red]-c-a, --config-app  : string  [white]] Load the specified app config. If the specified config cannot be found, returns to default.[/b]"
            )
            terminal.print_hr()
            terminal.print(
                f"[b][[red]-c-b, --config-bot  : string  [white]] Load the specified bot config, Also sets as default.\n{__spaces__(33)}If the specified config cannot be found, return to previous config as a precaution.[/b]"
            )
            terminal.print_hr()
            terminal.print(
                f"[b][[red]-d  , --debug       : bool    [white]] Enable debug mode for more information to be displayed in the terminal. Changes log mode to DEBUG.[/b]"
            )
            terminal.print_hr()
            terminal.print(
                f"[b][[red]-n  , --new         : empty   [white]] Manually put HangoutCore in a setup state in order to create a new configuration.\n{__spaces__(33)}Sets the new config as default.[/b]"
            )
            terminal.print_hr()
            terminal.print(
                f"[b][[red]-s-c, --system-cogs : bool    [white]] Enable/Disable to use provided system modules.[/b]"
            )
            terminal.print_hr()
            terminal.print(
                f"[b][[red]-s  , --silent      : bool    [white]] Enable/Disable to have information sent to log only, or terminal and log().\n{__spaces__(33)}Useful for running as a service since there's no access to terminal.[/b]"
            )
            terminal.print_hr()
            terminal.print(
                f"[b][[red]-t  , --token       : integer [white]] Specify which token to use if you're using multiple. Allows user to skip token choice prompt.[/b]"
            )
            terminal.print_hr()
            sys.exit(1)
        ...
        if len(_invalidArgs) > 0 or len(argv) > 0:
            ...
            log.INFO(
                f"Your provided args are listed below, Please double check your input and try again:\n hangoutcore {sys.argv[1:]}"
            )

            if not terminal.confirm(
                "[[red]CONFIRMATION REQUIRED[/red]] Would you like to continue startup?",
                default=False,
            ):
                sys.exit(1)
